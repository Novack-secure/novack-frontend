...
interface ServerMessage {
  ...
  created_at?: string;
  sender?: Participant;
  sender_employee?: Participant;
  sender_visitor?: Participant;
}
...
function mapServerRoom(r: ServerRoom): ChatRoom {
  const mapType = (t: string | undefined): "private" | "group" | "supplier" => {
    if (typeof t === "string") {
      const typeLower = t.toLowerCase();
      if (typeLower.includes("supplier") || typeLower.includes("group"))
        return "supplier";
...
class WebSocketService {
  ...
  sendMessage(
    ...
    return new Promise((resolve, reject) => {
      ...
      this.socket!.emit("sendMessage", { roomId, content }, (response: ServerResponse<Message>) => {
        clearTimeout(timeout);
        ...
        if (response.success === false || response.error) {
          reject(new Error(response.error || response.message || "Error desconocido"));
        } else {
          resolve(response);
        }
      });
   ...
   */
  joinRoom(roomId: string): Promise<ServerResponse> {
    if (!this.socket || !this.socket.connected) {
      return Promise.reject(new Error("WebSocket no está conectado"));
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      this.socket!.emit("joinRoom", { roomId }, (response: ServerResponse) => {
        clearTimeout(timeout);
   ...
   */
  leaveRoom(roomId: string): Promise<ServerResponse> {
    if (!this.socket || !this.socket.connected) {
      return Promise.reject(new Error("WebSocket no está conectado"));
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      this.socket!.emit("leaveRoom", { roomId }, (response: ServerResponse) => {
        clearTimeout(timeout);
    ...
    return new Promise((resolve) => {
      ...
      const timeout = setTimeout(() => {
        if (!hasResponded) {
          hasResponded = true;
          console.warn(
            "⏱️ Timeout al obtener mensajes después de 15s - retornando mensajes vacíos",
          );
      ...
      this.socket!.emit(
        "getRoomMessages",
        { roomId, limit, cursor },
        (response: ServerResponse<{ messages: ServerMessage[]; hasMore?: boolean; nextCursor?: string | null }>) => {
          if (hasResponded) {
            console.warn(
              "⚠️ Respuesta recibida después del timeout, ignorando",
            );
            return;
          }
          ...
          if (!response) {
            ...
            console.warn(
              "⚠️ No se recibió respuesta, retornando mensajes vacíos",
            );
            resolve({
              messages: [],
              hasMore: false,
          ...
          if (response.status === "error" || response.error) {
            ...
            const messagesData = response.data as { messages?: ServerMessage[]; hasMore?: boolean; nextCursor?: string | null } | undefined;
            const messages = (messagesData?.messages || response.messages || []).map(mapServerMessage);
            console.log(
              "🔍 getRoomMessages mensajes mapeados:",
              messages.length,
              "hasMore:",
              messagesData?.hasMore ?? response.hasMore,
            );
            resolve({
              messages,
              hasMore: messagesData?.hasMore ?? response.hasMore ?? false,
              nextCursor: messagesData?.nextCursor ?? response.nextCursor ?? null,
            });
          }
        },
      );
  ...
  createPrivateRoom(
    ...
    return new Promise((resolve, reject) => {
      ...
      this.socket!.emit(
        ...
        (response: ServerResponse<ServerRoom>) => {
          clearTimeout(timeout);
          ...
          if (response.success === false || response.error) {
            ...
            // Mapear sala del backend al formato frontend
            const room = mapServerRoom((response.data || response) as ServerRoom);
   ...
   */
  markMessagesAsRead(roomId: string): Promise<ServerResponse> {
    if (!this.socket || !this.socket.connected) {
      return Promise.reject(new Error("WebSocket no está conectado"));
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      this.socket!.emit("markAsRead", { roomId }, (response: ServerResponse) => {
        clearTimeout(timeout);
  ...
  onNewMessage(callback: (message: Message) => void) {
    ...
    this.socket.on("newMessage", (raw: ServerMessage) => {
  ...
  onRoomCreated(callback: (room: ChatRoom) => void) {
    ...
    this.socket.on("roomCreated", (raw: ServerRoom) => {
  ...
  onRoomUpdate(callback: (room: ChatRoom) => void) {
    ...
    this.socket.on("roomUpdate", (raw: ServerRoom) => {
      try {
