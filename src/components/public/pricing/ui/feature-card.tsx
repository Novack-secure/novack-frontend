    ...
    >
      ...
      <div className="relative z-10">
        <div className="text-center mb-6">
          <div
            className={`w-16 h-16 bg-linear-to-r ${feature.gradient} rounded-2xl flex items-center justify-center mx-auto mb-4 group-hover:scale-110 transition-transform duration-300 shadow-lg`}
          >
            <svg
              className={`w-8 h-8 ${
        ...
        <div className="space-y-3 mb-6">
          {feature.highlights.map((highlight, idx) => (
            <div key={idx} className="flex items-center space-x-3">
              <div className="shrink w-2 h-2 bg-[#0386D9] rounded-full"></div>
            ...
            <div className="grid grid-cols-2 gap-4">
              {feature.stats.map((stat, idx) => (
                <div key={idx} className="text-center">
                  <div className="text-lg font-bold text-[#0386D9] mb-1">
                    {stat.value}
                  </div>
          ...
          <div className="border-t border-white/10 pt-4">
            ...
            <div className="flex flex-wrap gap-2">
              {feature.certifications.map((cert, idx) => (
                <span
                  key={idx}
                  className="text-xs bg-[#0386D9]/20 text-[#0386D9] px-2 py-1 rounded-full font-medium"
                >
                  {cert}
                </span>
              ))}
